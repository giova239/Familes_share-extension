                ...
                if(credentials.validate(surname)){
                    if(emailvalidator.validate(email)){
                        if(pwdValidator.validate(password)){
                            if (arr.length != 0) {return res.status(400).json({ error: "Email already there, No need to register again.",});}
                            else {
                        ...
                        }else{res.status(400).json({error: 'Invalid Password, the password must be 8 characters long and have 1 uppercase, 1 lowercase character and 1 number!'});}
                    }else{res.status(400).json({error: 'Invalid Email. Please insert a valid email format!'});}
                }else{res.status(400).json({error: 'Surname cannot be empty. Please enter a valid surname!'});}
           }else{res.status(400).json({error: 'Date cannot be empty. Please enter a valid date!'});}
        }else{res.status(400).json({error: 'Name cannot be empty. Please enter a valid name!'});}
    }
    catch (err) {
        console.log(err);
        res.status(500).json({
